...
from packages.diffusion_motion_generation.trainer_interaction_motion_diffusion import run_train, run_validation, run_sample
import torch 
...
def main(cfg: DictConfig):
    ...
    OmegaConf.set_struct(cfg, False) # this is done to allow keys that are not in the original config dict to be added, in case such a functionality is needed
    if cfg.algorithm.name == 'diff_trans':
        cfg.algorithm.save_dir = os.path.join(cfg.algorithm.project, cfg.algorithm.exp_name)
        device = torch.device(f"cuda:{cfg.algorithm.device}" if torch.cuda.is_available() else "cpu")
    ...
    from flappy_bird_gym.utils.group_trajs import group_trajectories, add_pseudo_latent_state
    ...
    grouped_trajs = group_trajectories(trajectories, group_criteria=cfg.group_criteria, group_size=cfg.group_size, num_groups=cfg.num_groups)
    ...
    policy = TrainerAllTypes(train_envs, eval_envs, grouped_trajs, cfg.seed, cfg.algorithm, cfg).train()
    wandb.finish()
