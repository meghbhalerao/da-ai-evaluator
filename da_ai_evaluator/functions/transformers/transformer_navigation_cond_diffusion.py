...
from packages.diffusion_motion_generation.manip.inertialize.inert import apply_inertialize
from packages.diffusion_motion_generation.manip.lafan1.utils import quat_slerp, rotate_at_frame_w_obj_global
from packages.diffusion_motion_generation.manip.model.transformer_module import Decoder
from packages.diffusion_motion_generation.manip.utils.model_utils import wxyz_to_xyzw, xyzw_to_wxyz
...
class NavigationCondGaussianDiffusion(nn.Module):
    def __init__(
        ...
        self.clip_encoder = nn.Sequential(nn.Linear(in_features=512, out_features=512),)
    ...
    def p_sample_loop_sliding_window_w_canonical(
        ...
        if add_root_ori:
            ...
            root_xy_ori_mask = torch.ones(cond_mask.shape[0], cond_mask.shape[1], 6).to(cond_mask.device)

            root_xy_ori_mask[:, [0, 30 - 1, 60 - 1, 90 - 1, 120 - 1], :] = 0

            cond_mask = torch.cat((cond_mask[..., :204], root_xy_ori_mask, cond_mask[..., 204:]), dim=-1)
        ...
        for t_idx in tqdm(range(0, num_steps, stride)):
            ...
            if t_idx == 0:
                ...
                x_pose_cond = curr_x_start * (
                    1.0 - cond_mask)  # Remove noise, overall better than adding random noise.
                ...
                whole_sample_res = whole_sample_res[:, :-overlap_frame_num, :]  # BS X 10 X D
                ...
                def use_T_start_pose():
                    ...
                    global_quat, global_pos = quat_fk_torch(
                        local_joint_rot_mat.cuda(), curr_seq_local_jpos.cuda(), data_root_folder=self.opt.data_root_folder
                    )
    ...
    def p_losses(
        ...
        if self.add_feet_contact:
            ...
            data_root_folder=self.opt.data_root_folder)  # (BS*T) X 22 X 3 X 3
