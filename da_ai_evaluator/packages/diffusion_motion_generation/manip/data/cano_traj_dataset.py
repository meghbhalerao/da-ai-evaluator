...
from packages.diffusion_motion_generation.manip.lafan1.utils import rotate_at_frame_w_obj
from packages.diffusion_motion_generation.manip.utils.visualize.tools.utils import contact_ids
...
def get_smpl_parents(use_joints24=True, data_root_folder = None):
    ...
    # print(data_dir)
    # sys.exit()
    assert data_root_folder is not None, "Data root folder is None! Please enter a filepath!"
    parents_22_path = os.path.join(data_root_folder, "smpl_parents_22.npy")
    parents_24_path = os.path.join(data_root_folder, "smpl_parents_24.npy")
...
def local2global_pose(local_pose, data_root_folder):
    # local_pose: T X J X 3 X 3
    kintree = get_smpl_parents(use_joints24=False, data_root_folder=data_root_folder)
...
def quat_ik_torch(grot_mat, data_root_folder):
    # grot: T X J X 3 X 3
    parents = get_smpl_parents(use_joints24=False, data_root_folder=os.path.join(data_root_folder, "..", 'smpl_all_models'))
...
def quat_fk_torch(lrot_mat, lpos, use_joints24=True, data_root_folder = None):
...
class CanoObjectTrajDataset(Dataset):
    def __init__(
        ...
        self.parents = get_smpl_parents(data_root_folder=os.path.join(self.data_root_folder, "../", "smpl_all_models" ))  # 24/22
        ...
        if self.load_ds:
            if os.path.exists(processed_data_path):
                ...
                # print(self.window_data_dict[0].keys())
                # print(self.window_data_dict[0]['seq_name'])
                # print(self.window_data_dict[0]['seq_name'].shape)
                # sys.exit()
                # for k, v in self.window_data_dict[0].items():
                #     try:
                #         try:
                #             print(k, v.shape)
                #         except:
                #             print("Does not have shape attribute")
                #         try:
                #             print(k, len(v))
                #         except:
                #             print("Does not have length attribute")
                #     except:
                #         print(f"Does not have shape or length, just skipping this key {k}, {type(k)}")
    ...
    def process_window_data(
        ...
        global_joint_rot_mat = local2global_pose(local_joint_rot_mat, data_root_folder=self.data_root_folder)  # T' X 22 X 3 X 3
        ...
        _, human_jnts = quat_fk_torch(local_joint_rot_mat, curr_seq_local_jpos, data_root_folder=self.data_root_folder)
