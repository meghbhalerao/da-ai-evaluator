    ...
    for traj_file in os.listdir(data_path):
        ...
        if traj_file.endswith('.pkl'):
            with open(os.path.join(data_path, traj_file), 'rb') as f:
                ...
                infos = [{"state": state} for state in states][:-1]
                ...
                print(f"Length of obs: {len(obs)}, actions: {len(actions)}, infos: {len(infos)}, rewards: {len(rewards)}") # printing the lenghths of individual items in in the trajectory class
